...
url = 'http://www.tlbts.com/votepic/infovote_action.php?tid=1&radiobutton=600'
vote_data = {'id': '678998'}
...
def vote(proxy_addr):
    ...
    try:
        ...
        if r.status_code == requests.codes.ok:
            print(proxy_addr, '\t', json.loads(r.text))
        else:
...
def main():
    while True:
        with open('proxy.txt', 'r+', encoding='utf-8') as f:
            proxyaddrs = [''.join('http://', line.split('@', 1)[0]) for line in f.readlines()]
            try:
                pool = ThreadPool(8)
